        ...
        Machine.create(answers, function (err, machine) {
          ...
          Machine.execTty(machine, command, args, function (err) {
            ...
            Machine.setCurrent(machine, function (err, machine) {
              ...
              // All done!
              glushAscii.done(machine.name + ' VM started and set to current machine!', true);
...
program
  ...
  .action(function () {
    return Machine.list(function (err, machines) {
      if (err) return logger.error(err);
...
program
  ...
  .action(function (name) {
    ...
    return Machine.getCurrent(function (err, machine) {
      if (err) return logger.error(err);

      // All done!
      logger.info('Current machine is `' + machine.name +'`\n');
    });
  });
  ...
  .action(function (name) {
    Machine.get(name, function (err, machine) {
      ...
      Machine.exec(machine, 'vagrant halt', function (err) {
        ...
        // All done!
        glushAscii.done('', true);
        logger.info(machine.name + ' VM stopped!\n');
      });
    });
  });
  ...
  .action(function (name, options) {
    Machine.get(name, function (err, machine) {
      ...
      Machine.exec(machine, command, function (err) {
        ...
        // All done!
        glushAscii.done('', true);
        logger.info(machine.name + ' VM folder ' + mode + ' completed!\n');
      });
    });
  });
  ...
  .action(function (name, options) {
    Machine.get(name, function (err, machine) {
      ...
      Machine.execTty(machine, command, args, function (err) {
        ...
        // All done!
        glushAscii.done('', true);
        logger.info(machine.name + ' VM ssh session completed!\n');
      });
    });
  });
  ...
  .action(function (name) {
    Machine.get(name, function (err, machine) {
      ...
      Machine.exec(machine, command, function (err) {
        ...
        // All done!
        glushAscii.done('', true);
        logger.info(machine.name + ' VM processes killed!\n');
      });
    });
  });
        ...
        Machine.destroy(machine.name, function (err, deleted) {
          ...
          if (current && current.name === machine.name) {
            Machine.setCurrent({}, function (err, current) {
              ...
              // All done!
              glushAscii.done('', true);
              logger.info(machine.name + ' VM Destroyed!\n');
            });
          }
        });
      });
