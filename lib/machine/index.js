...
/**
 * SSH into a VM and perform a command
 */
program
  .command('ssh-command [cmd] [name]')
  .description('SSH into a VM')
  .action(function (cmd, name) {
    ...
    Machine.get(name, function (err, machine) {
      ...
      Machine.execTty(machine, command, args, function (err) {
        ...
        // All done!
        inquisitorAscii.done('', true);
        return logger.info(machine.name + ' VM ssh command completed!\n');
      });
    });
  });
...
program
  ...
  .option('-r, --reset-key', 'Reset the Rancher API key')
  .action(function (name, options) {
    if (!options['export']) {
      logo('small', logoMessage);
    }
