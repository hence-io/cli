...
module.exports = function (next) {
  ...
  var dependancies = [
    'docker|1.10.3',
    'rancher-compose|v0.7.3-rc2|gte|v'
  ];
      ...
      switch (name) {
        case 'docker':
          installCommand = 'rm -f /usr/local/bin/docker && ' +
              'curl -L https://get.docker.com/builds/Darwin/x86_64/docker-1.10.3 > /usr/local/bin/docker && ' +
              'chmod +x /usr/local/bin/docker';
          break;

        case 'rancher-compose':
          installCommand = 'rm -f /usr/local/bin/rancher-compose && ' +
              'curl -L https://github.com/rancher/rancher-compose/releases/download/v0.7.3-rc1/rancher-compose-darwin-amd64-v0.7.3-rc1.tar.gz | ' +
