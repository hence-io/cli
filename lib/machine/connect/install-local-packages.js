...
module.exports = function (next) {
  ...
  var dockerV = '1.11.2';
  var rancherComposeV = 'v0.8.5';

  var dependancies = [
    'docker|' + dockerV,
    'rancher-compose|' + rancherComposeV + '|gte|v'
  ];
      ...
      switch (name) {
        case 'docker':
          installCommand = 'rm -f /usr/local/bin/docker && ' +
              'curl -L https://get.docker.com/builds/Darwin/x86_64/docker-' + dockerV + ' > /usr/local/bin/docker && ' +
              'chmod +x /usr/local/bin/docker';
          break;

        case 'rancher-compose':
          installCommand = 'rm -f /usr/local/bin/rancher-compose && ' +
              'curl -L https://github.com/rancher/rancher-compose/releases/download/' + rancherComposeV + '/rancher-compose-darwin-amd64-' + rancherComposeV + '.tar.gz | ' +
