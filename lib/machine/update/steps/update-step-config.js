  ...
  prompts: [
    {
      ...
      "default": (defaults.name ? defaults.name : vagrantDefaults.name),
    ...
    },
    {
      ...
      "default": (defaults.ip ? defaults.ip : vagrantDefaults.ip),
    ...
    },
    {
      ...
      "default": (defaults.port ? defaults.port : vagrantDefaults.port),
    ...
    },
    {
      ...
      "default": (defaults.cpus ? defaults.cpus : vagrantDefaults.cpus),
      validate: inquisitor.inquirer.validatePrompt('isInt', 'You must specify an integer')
    },
    {
      name: 'memory',
      message: 'How much memory (in MB) will you allocate to this VM?',
      "default": (defaults.memory ? defaults.memory : vagrantDefaults.memory),
      validate: inquisitor.inquirer.validatePrompt('isInt', 'You must specify an integer')
    },
    {
      name: 'nodes',
      message: 'How many nodes (clustered VM\'s) will you run? 1 is typical for local development.',
      "default": (defaults.nodes ? defaults.nodes : vagrantDefaults.nodes),
      validate: inquisitor.inquirer.validatePrompt('isInt', 'You must specify an integer')
    }
  ],
