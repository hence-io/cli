  ...
  _.extend(self, {
    ...
    setMachines: function setMachines (machines, next) {
      if (_.isPlainObject(machines)) {
        ...
        return fs.outputJson(global.vm.machinesFile, sortedMachines, jsonWriteOptions, function (err) {
    ...
    setCurrent: function setCurrent (current, next) {
      ...
      var _setCurrent = function (machine, next) {
        ...
        return fs.outputJson(global.vm.currentMachineFile, machine, jsonWriteOptions, function (err) {
          if (err) return next(err);
     ...
     */
    updateConfig: function updateeConfig (config, machine, next) {
      var machines = self.getMachines();
      ...
      // Save API key if present in old record
      var apiKey = machines[machine.name].apiKey || false;
      if (_.isPlainObject(apiKey) && !_.isPlainObject(config.apiKey)) {
    ...
    /**
     * Start a machine wizard of a given type
    ...
    getDashboardPath: function getDashboardPath (config, useIpAddress) {
      var host = useIpAddress ? config.ip : config.name;

      return ['http://', host, ':', config.port, '/'].join('');
    },

    openDashboard: function openDashboard (machine, useIpAddress) {
      var dashboardPath = self.getDashboardPath(machine, useIpAddress);

      // Open up the Dashboard in browser
      exec('open ' + dashboardPath);

      logger.info(machine.name + ' dashboard opened at: ' + dashboardPath + '\n');
    },
    ...
    generateApiKey: function generateApiKey (config, data, next) {
      ...
      if (_.isFunction(data)) {
        next = data;
        data = {
          description: 'Local Hence CLI Key',
          name: 'hence-cli'
        };
      }
